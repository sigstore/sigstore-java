...
public class RekorV2ClientHttpTest {
  ...
  public void putEntry() throws Exception {
    ...
    var keyIdBytes = entry.getLogId().getKeyId().toByteArray();
    var base64EncodedKeyId = java.util.Base64.getEncoder().encodeToString(keyIdBytes);
    System.out.println(
        "The keyId for this entry should be 8w1amZ2S5mJIQkQmPxdMuOrL/oJkvFg9MnQXmeOCXck= and is:");
    System.out.println(base64EncodedKeyId);
    ...
    try {
      ...
      Map<String, Object> entryMap = GsonSupplier.GSON.get().fromJson(entryJson, type);

      jsonPayload = GsonSupplier.GSON.get().toJson(entryMap);
    } catch (InvalidProtocolBufferException e) {
      throw new RekorParseException("Failed to serialize HashedRekordRequestV002 to JSON", e);
    }
    System.out.println("This is the entry: ");
