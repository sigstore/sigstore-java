...
public class Serialization {
   ...
   * The vector is prefixed by its total length, in bytes and in big endian format, so is each
    ...
    try {
      ...
      if (count < length) {
        throw new SerializationException(
            "Premature end of input, expected " + length + " bytes, only read " + count);
      }
      return data;
    } catch (IOException e) {
      throw new SerializationException(e);
  ...
  /**
   * Read a number in big endian format from the input stream. This methods only supports a width of
   * up to 4 bytes.
   *
  ...
  /**
   * Read a number in big endian format from the input stream. This methods supports a width of up
   * to 8 bytes.
   *
   * @param width the width of the number, in bytes.
   * @throws SerializationException if EOF is encountered.
  ...
  /**
   * Write length prefixed sequence of bytes to the ouput stream. The length prefix is encoded in
   * big endian order.
   *
   * @param data the data to be written.
   * @param width the width of the length prefix, in bytes.
   * @throws SerializationException if the length of {@code data} is too large to fit in {@code
   *     width} bytes or {@code width} is negative.
   */
