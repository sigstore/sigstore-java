...
cd tmp
# modify the targets to have a version which doesn't match the targets version in snapshot.
jq -r '.signed.version |= 2' repository/targets.json | sponge repository/targets.json
...
# update and resign the timestamp.json with the new snapshot.json hash and size
